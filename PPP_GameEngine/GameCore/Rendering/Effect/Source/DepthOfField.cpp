...
namespace
{
	...
	ComputePipelineState s_VerticalPipeline(L"Blur::VerticalPipelineState");
	ComputePipelineState s_RhomboidPipeline(L"Blur::RhomboidPipelineState");
	ComputePipelineState s_FinalRenderPipeline(L"Dof::FinalRenderPipelineState");
...
void Dof::OnResize(float newWidth, float newHeight)
{
	for (auto& buffer : _colorBuffer)
	{
		buffer.OnResize(newWidth, newHeight, 1);
	}
}
/****************************************************************************
*							Draw
...
void Dof::Draw(GPUResource* renderTarget, GPUResource* zPrepass)
{
	...
	context->Dispatch(_colorBuffer[0].GetColorBuffer().get()->PixelSize.x / 16, _colorBuffer[0].GetColorBuffer().get()->PixelSize.y / 16, 1);
	/*-------------------------------------------------------------------
	-               Rhomboid Blur
	---------------------------------------------------------------------*/
...
{
	auto& engine = GraphicsCoreEngine::Instance();
	_blurParameterBuffer = std::make_unique<UploadBuffer>(engine.GetDevice(), sizeof(BlurParameter), 1, true, _addName + L"BlurParameter");
...
{
	auto& engine    = GraphicsCoreEngine::Instance();
	_clipSizeBuffer = std::make_unique<UploadBuffer>(engine.GetDevice(), sizeof(ClipSize), 1, true, _addName + L"ClipSize");
...
void Dof::PreparePipelineState()
{
	...
	s_VerticalPipeline   .CompleteSetting(device);
	s_RhomboidPipeline   .CompleteSetting(device);
	s_FinalRenderPipeline.CompleteSetting(device);
}
