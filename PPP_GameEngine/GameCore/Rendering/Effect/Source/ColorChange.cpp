...
void ColorChange::PrepareVertexAndIndexBuffer()
{
	...
	for (int i = 0; i < totalFrameSize; ++i)
	{
		...
		---------------------------------------------------------------------*/
		meshBuffer.VertexBuffer = std::make_unique<UploadBuffer>(engine.GetDevice(), vertexByteSize, vertexCount, false, _addName + L"VertexBuffer");
		...
		meshBuffer.IndexBuffer  = std::make_unique<UploadBuffer>(engine.GetDevice(), indexByteSize, indexCount, false, _addName + L"IndexBuffer");
...
void ColorChange::PreparePipelineState(ColorChangeType type)
{
	...
	---------------------------------------------------------------------*/
	auto pipelineState = std::make_unique<GraphicsPipelineState>(L"ColorChangePSO");
	...
	pipelineState->CompleteSetting(device->GetDevice());
