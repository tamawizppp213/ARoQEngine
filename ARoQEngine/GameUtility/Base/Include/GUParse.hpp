...
namespace gu
{
	...
	*   @brief   Value -> 読み込み順序が決定されていないものを探すときに使用します.
    *   @brief   文字列全体を探し, Matchに合う文字列の箇所の後に続く数値を読み取ります. (DesireToFind=100)
	*	@brief	 Trueの場合の認識文字列はTrue, Yes, On, 1 (大文字小文字区別なし)
    *   @brief   Falseの場合の認識文字列はFalse, No, Off, 0 (大文字小文字区別なし)
	*****************************************************************************/
	class Parse final
	{
	public:
		...
		*  @brief 型の判定は引数に入れた型で自動判別してしまうため, 事前に型は決めておいてください. 
		----------------------------------------------------------------------*/
		...
		*  @brief     検索にひっかかかる文字列は以下の通りです
		*  @brief     True  : True, Yes, On, 1
		*  @brief     False : False, No, Off, 0 
		*  
