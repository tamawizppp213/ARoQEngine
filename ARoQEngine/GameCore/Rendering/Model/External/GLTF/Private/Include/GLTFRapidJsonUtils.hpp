...
#pragma once
#ifndef GLTF_RAPID_JSON_UTILS_HPP
#define GLTF_RAPID_JSON_UTILS_HPP
...
#include "GameUtility/File/External/rapidjson/schema.h"
#pragma warning(pop)
...
#include "GameUtility/File/External/rapidjson/error/en.h"
#include "GameUtility/File/External/rapidjson/istreamwrapper.h"
#include "GameUtility/File/External/rapidjson/pointer.h"
#include "GameUtility/File/External/rapidjson/prettywriter.h"
#include "GameUtility/File/External/rapidjson/stringbuffer.h"

//#include "GameUtility/File/External/GLTFSDK/Color.h"
#include "GLTFExceptions.hpp"
#include "GLTFIndexedContainer.hpp"
...
namespace gltf
{
    ...
    *  @param[in] const char* memberName
    *  @return 　　std::string
    *****************************************************************************/
    template<typename T>
    std::string GetMemberValueAsString(const rapidjson::Value& v, const char* memberName)
    {
        ...
        if (it != v.MemberEnd()) { return std::to_string(it->value.Get<T>()); }
        else                     { return {}; }
    }
    /****************************************************************************
    *                          GetMemberValueAsString
    ...
    *****************************************************************************/
    ...
    {
        auto it = v.FindMember(memberName);
        if (it != v.MemberEnd()) { return std::to_string(it->value.Get<KnownSizeType>()); }
        else { return {}; }
    }
    /****************************************************************************
    *                          TryFindMember
    ...
    *  @param[in] rapidjson::Value& v
    *  @param[in] rapidjson::Value::ConstMemberIterator& value
    *  @return 　　bool
    *****************************************************************************/
    ...
    *  @param[in] cosnt char* memberName
    *  @param[in] const rapidjson::Value& v
    *  @return 　　rapidjson::Value::ConstMemberIterator
    *****************************************************************************/
    inline rapidjson::Value::ConstMemberIterator FindRequiredMember(const char* name, const rapidjson::Value& v)
    {
        ...
        if (it == v.MemberEnd())
        {
            throw gltf::detail::error::InvalidGLTFException("The member " + std::string(name) + " was not found");
        }
        return it;
    }
    /****************************************************************************
    ...
    *****************************************************************************/
    ...
    namespace RapidJsonUtils
    {
        ...
        /*inline glTF::rapidjson::Value ToJsonArray(const gm::Float2& v, rapidjson::Document::AllocatorType& a)
        {
            rapidjson::Value vArray(rapidjson::kArrayType);
        ...
        inline rapidjson::Value ToJsonArray(const gm::Float3& v, rapidjson::Document::AllocatorType& a)
        {
            rapidjson::Value vArray(rapidjson::kArrayType);
            vArray.PushBack(v.x, a);
            vArray.PushBack(v.y, a);
        ...
        inline rapidjson::Value ToJsonArray(const gm::Float4& v, rapidjson::Document::AllocatorType& a)
        {
            rapidjson::Value vArray(rapidjson::kArrayType);
            vArray.PushBack(v.x, a);
            vArray.PushBack(v.y, a);
        ...
        template <typename T>
        inline void AddOptionalMemberIndex(const std::string& name, rapidjson::Value& v, const std::string& id, const gltf::IndexedContainer<const T>& container, rapidjson::Document::AllocatorType& a)
        {
            if (!id.empty())
            {
        ...
        template <typename T>
        inline void AddOptionalMemberIndexChild(const std::string& name, const std::string& childName, rapidjson::Value& v, const std::string& id, const gltf::IndexedContainer<const T>& container, rapidjson::Document::AllocatorType& a)
        {
            if (!id.empty())
            {
        ...
        inline rapidjson::Document CreateDocumentFromString(const std::string& json)
        {
            ...
            if (document.Parse(json.c_str()).HasParseError())
            {
                ...
                throw gltf::detail::error::GLTFException("The document is invalid due to bad JSON formatting");
            }
