...
///          エンジンを再起動しても情報を保持することができるクラス.Json形式で保存します.暗号化まではまだ行いません @n
///          使い方は読み込み時はLoadを事前に呼び出し, その後に読み込みたい変数を<< MAKE_ARCHIVE_PAIRで登録します. @n
///          保存時は保存したい変数を<< MAKE_ARCHIVE_PAIRで登録した後, Saveを呼び出します. @n
///  @author toide
